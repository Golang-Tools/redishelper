...
func Test_cuckoofilterpipe_ADD(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe"
	pipe := cli.Pipeline()
...
func Test_cuckoofilterpipe_ADD_ttl(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-ttl"
	pipe := cli.Pipeline()
...
func Test_cuckoofilterpipe_MADD(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-madd"
	pipe := cli.Pipeline()
...
func Test_cuckoofilterpipe_MADD_ttl(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-madd"
	pipe := cli.Pipeline()
...
func Test_cuckoofilterpipe_Reserve_Insert(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-ri"
	pipe := cli.Pipeline()
	ReservePipe(pipe, ctx, key, 1000)
...
func Test_cuckoofilterpipe_Reserve_Insert_ttl_noscal(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-ri-noscal"
	pipe := cli.Pipeline()
	ReservePipe(pipe, ctx, key, 1000, ReserveWithTTL(2*time.Second))
...
func Test_cuckoofilterpipe_Insert_nocreateerr(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-ri-nocreateerr"
	pipe := cli.Pipeline()
...
func Test_cuckoofilterpipe_Insert(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-ri-nocreateerr"
	pipe := cli.Pipeline()

	inser1, err := InsertPipe(pipe, ctx, key, []string{"abc", "cde", "def"}, InsertWithCapacity(1000), InsertWithExpansion(2), InsertWithTTL(2*time.Second))
	if err != nil {
		assert.FailNow(t, err.Error(), "InsertPipe error")
	}
	exist1, err := MExistsItemPipe(pipe, ctx, key, "abc", "cde", "def")
...
func Test_Cuckoofilterpipe_Count_DEL(t *testing.T) {
	cli, ctx := NewBackground(t, TEST_REDIS_URL)
	defer cli.Close()
	key := "test-cuckoofilter-pipe-cd"
	pipe := cli.Pipeline()

	//一次设置
	_, err := AddItemPipe(pipe, ctx, key, "abc", AddWithoutNX())
	if err != nil {
	...
	//计数1
	res1 := CountItemPipe(pipe, ctx, key, "abc")
	//删除1
	res2 := DelItemPipe(pipe, ctx, key, "abc")
	//计数2
	res3 := CountItemPipe(pipe, ctx, key, "abc")
	//删除2
	res4 := DelItemPipe(pipe, ctx, key, "abc")
	//计数3
	res5 := CountItemPipe(pipe, ctx, key, "abc")
	//删除3
	res6 := DelItemPipe(pipe, ctx, key, "abc")
	//计数4
	res7 := CountItemPipe(pipe, ctx, key, "abc")
	//删除4
	res8 := DelItemPipe(pipe, ctx, key, "abc")
	_, err = pipe.Exec(ctx)
	if err != nil {
		assert.FailNow(t, err.Error(), "pipe.Exec error")
	}
